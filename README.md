...
> [!NOTE]
> **Status: latest build is live — [v0.1.0-pre.9](https://github.com/clearcotelabs/clearcote-browser/releases/tag/v0.1.0-pre.9) (Chromium 149, Windows x64).**
> A signed, checksummed pre-release build is available now on the [Releases page](https://github.com/clearcotelabs/clearcote-browser/releases) — download it, [verify it](docs/VERIFY.md), and run it. It centers on **fingerprint profile import**: present *that machine's* GPU, screen, fonts, speech voices, audio, and WebGL by passing a captured profile via `--fingerprint-profile` / the SDK's `fingerprint_profile` option (fields you don't import fall back to the `--fingerprint` seed, so partial profiles stay coherent). Get a profile from the ready-made **[clearcote-profiles](https://github.com/clearcotelabs/clearcote-profiles)** library, the [collector](tools/fingerprint-collect), or the [10k-profile dataset](https://github.com/Vinyzu/chrome-fingerprints); then confirm it loaded with [`verify_profile.py`](tools/fingerprint-collect/verify_profile.py). (pre.9 is engine-identical to pre.8 — same binary checksum — and adds the curated library + verification tooling + docs.) Still **passes open-source fingerprint auditors** (`navigator.webdriver=false`, 0% headless / 0% stealth) on real Windows — an experimental pre-release; see the [Roadmap](ROADMAP.md) for what's next.
