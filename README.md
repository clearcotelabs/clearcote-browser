...
> **Status:** [**v0.1.0-pre.18**](https://github.com/clearcotelabs/clearcote-browser/releases/tag/v0.1.0-pre.18) is live — Chromium 149, **Windows x64 + Linux x64**, signed + checksummed ([verify it](docs/VERIFY.md)). **New — coherence-first TLS network persona:** `--fingerprint-tls-profile=chrome-<major>` (SDK `tlsProfile`, default `match-persona`) makes the TLS ClientHello follow the persona's *claimed* Chrome version — the post-quantum key-share group (MLKEM ≥131 / Kyber 124–130 / none <124) and the ALPS codepoint match that major, while the cipher list, version bounds, and per-connection extension permutation stay exactly real-Chrome (a fixed JA3 is itself a tell). Chromium-core (Chrome/Edge/Brave/Opera); coherence, not arbitrary spoofing. All prior surfaces remain: native **Windows + Linux** personas (Linux-native GPU/voice/audio-device values), opt-in **Widevine / EME (DRM)** (`widevine=True`), the per-origin **[canvas bridge](docs/CANVAS-BRIDGE.md)** (`--canvas-bridge-mode`), the continuous **humanized cursor** (`Browser.humanizedClick`), real-fingerprint import, and the **[stealth-coherence regression gate](docs/STEALTH-COHERENCE.md)** that runs every release. The SDK (`clearcote` ≥ 0.12.0) auto-downloads the right binary per OS. macOS is on the [Roadmap](ROADMAP.md). An experimental pre-release.
