...
> **Status:** [**v0.1.0-pre.16**](https://github.com/clearcotelabs/clearcote-browser/releases/tag/v0.1.0-pre.16) is live — Chromium 149, Windows x64, signed + checksummed ([verify it](docs/VERIFY.md)). **Fingerprint-coherence hardening**: `measureText()` and the client-rect APIs now return truthful, on-grid values — text metrics sit on Chrome's native 1/512-px grid, `measureText` agrees between the main thread and an `OffscreenCanvas` worker, and `getBoundingClientRect` agrees with `Range` rects. A new **[stealth-coherence regression gate](docs/STEALTH-COHERENCE.md)** runs on every release so these invariants can't silently regress. All prior surfaces remain: opt-in **Widevine / EME (DRM)** (`widevine=True`), the per-origin **[canvas bridge](docs/CANVAS-BRIDGE.md)** (`--canvas-bridge-mode`), and the continuous **humanized cursor** (`Browser.humanizedClick`). **🐧 New — now on Windows x64 AND Linux x64:** [**`v0.1.0-pre.17`**](https://github.com/clearcotelabs/clearcote-browser/releases/tag/v0.1.0-pre.17) adds a signed Linux x64 build (de-Googled Chromium 149 with the full coherent-persona stack — the persona now emits Linux-native GPU/voice/audio-device values, and DRM works via the Linux CDM). The SDK (`clearcote` ≥ 0.11.0) auto-downloads the right binary per OS. macOS is on the [Roadmap](ROADMAP.md). An experimental pre-release.
