...
- [x] **Cross-surface coherence pass** *(v0.1.0-pre.6)*: coherent WebGL `getParameter` limits + session-constant GPU; `navigator` battery/connection/keyboard; audio, screen & `getScreenDetails()` metadata; CSS pointer/hover; OS-coherent `URL`; `navigator.share`/`canShare` — validated on BrowserScan ("Normal") + CreepJS (0% headless / 0% stealth)
